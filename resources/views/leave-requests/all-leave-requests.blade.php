            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg py-3 ">
                ...
                <div class="flex justify-center max-w-4xl mx-auto sm:px-6 lg:px-8">
                    <nav class="nav nav-pills nav-fill nav-justified border border-primary rounded py-1 px-1 w-100">
                        <a href="{{ route('all_leave_requests') }}" class="w-25 nav-item "><button type="button" class="btn btn-primary text-white py-1  w-100 font-weight-bold "><span>All</span></button></a>
                        ...
                        <a href="{{ route('pending_leave_requests') }}" class="w-25 nav-item"><button type="button" class="btn py-1 w-100 "><span>Pending</span></button></a>
                ...
                <div class="flex justify-center">
                    <div class="p-4 w-full sm:w-5/6 list-group">
                        @foreach ($leaveRequestLists as $leaveRequestList)
                            ...
                            <div class="border border-primary rounded w-100 px-2 py-1 flex flex-col sm:flex-row items-center justify-between">
                                <div class="flex flex-col items-center sm:flex-row">
                                    <span class="text-capitalize badge w-24 {{ $leaveRequestList->status == 'approved' ? 'bg-success' : ($leaveRequestList->status == 'pending' ? 'bg-yellow-500' : 'bg-red-500') }}">{{$leaveRequestList->status}}</span> 
                                ...
                                <div class="flex items-center space-x-2">
                                    @if ($leaveRequestList->status == 'pending')
