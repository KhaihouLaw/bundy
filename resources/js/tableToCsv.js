(function($) {
    ...
    $.fn.tableToCsv = function(options) {
        ...
        {
            const rows = table.find('tr').not(settings.excludeRows);
            return [].slice.call(rows)
                .map(function(row) {
                    // const cells = row.querySelectorAll('th,td'); // original
                    const cells = $(row).find('th,td').not(settings.excludeColumns);
